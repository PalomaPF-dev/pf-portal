...
async function fetchUserProfile(sql, loginId) {
  const rows = await sql`
    SELECT u.id, u.login_id, u.name, u.password_hash, u.department_id, u.role, u.can_manage,
           d.name AS department_name, d.apps AS department_apps,
           w.name AS workplace_name,
           a.login_id AS approver_login_id, a.name AS approver_name
  ...
  return {
    ...
    // ※ role は承認フロー上の役割。管理画面に入れるかどうかは canManage（ポータル管理権限）で決まる。
    ...
    // 承認者（上司）。進捗管理(operation)などが残業申請の承認ルートに使う
    approverLoginId: u.approver_login_id || null,
    approverName: u.approver_name || null,
    apps,
...
function profileResponse(profile) {
  const out = {
    ...
    apps: profile.apps,
    // 本人の承認フロー上の役割。管理者専用アプリのタイルを出すかの判定に使う。
    // 本人自身の情報なので開示して差し支えない（他人の role は返していない）。
    role: profile.role,
  };
  if (profile.noDepartment) out.noDepartment = true;
  // ポータル管理権限を持つ本人にだけ、管理画面への導線を表示するためのフラグ
  if (profile.canManage) out.canManage = true;
  return out;
}
