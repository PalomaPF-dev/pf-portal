...
// 1アプリ分をまとめて呼ぶ。users: [{loginId,name,email?,factory?,role?,canManage?,approverLoginId?}]
// → [{loginId,status,inviteUrl,passwordSet}]
    ...
    const r = await fetch(provisionUrl(appKey), {
      ...
      body: JSON.stringify({
        ...
        users: users.map((u) => ({
          ...
          // 所属工場。アプリ側はこれが入っていると「その工場のデータだけ」に絞る。
          // ポータル管理権限を持つ人は全社を見る立場なので、明示的に null を送って
          // 工場の指定を外す（各アプリの「工場未設定＝全工場を閲覧可」の扱いになる）。
          // ※ undefined だと JSON からキーごと消え、アプリ側は「指定なし＝現状維持」と
          //   解釈するため、すでに工場が入っている人の制限が外れない。
          // 承認フロー上の管理者(role=admin)は各アプリ側で元から制限対象外。
          factory: u.canManage === true ? null : u.factory || undefined,
