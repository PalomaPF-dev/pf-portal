...
// ※ zumen は招待リンクを持たないため inviteUrl は常に null（SSO／アプリ側の初回パスワード設定で入る）。
// ※ アプリ側 API 未実装・鍵未設定（503 等）や タイムアウト/通信失敗 は status:'error' として記録。

const { appBaseUrl } = require("./appUrls");

const PROVISION_APP_KEYS = ["keikaku", "nippou", "sekisai", "zumen", "keisoku", "setsubi", "hinshitsu", "zaiko", "kanagata", "hoju", "tenchu", "purchasing"];
    ...
    const r = await fetch(provisionUrl(appKey), {
      ...
      body: JSON.stringify({
        ...
        users: users.map((u) => ({
          ...
          role: u.role === "admin" ? "admin" : u.role === "worker" ? "worker" : "member",
...
async function provisionUsers(sql, users) {
  ...
  for (const u of users) {
    ...
    for (const app of apps) {
      if (!PROVISION_APP_KEYS.includes(app)) continue; // 未知キーは除外
