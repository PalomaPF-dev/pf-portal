...
// role は 'admin' | 'member'、approverLoginId は承認者（管理者）の login_id または null。
// ※ zumen はアカウント不要のためプロビジョニング対象外。
// ※ アプリ側 API 未実装・鍵未設定（503 等）や タイムアウト/通信失敗 は status:'error' として記録。

const { appBaseUrl } = require("./appUrls");

const PROVISION_APP_KEYS = ["keikaku", "nippou", "sekisai", "keisoku", "setsubi", "hinshitsu", "zaiko", "kanagata", "hoju", "tenchu"];
...
// 1アプリ分をまとめて呼ぶ。users: [{loginId,name,email?,factory?,role?,approverLoginId?}] → [{loginId,status,inviteUrl}]
