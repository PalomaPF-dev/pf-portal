// ユーザー名簿 API（すべて管理者専用）。
// GET    一覧（部署・職場・承認者 join + 発行状況）
// POST   {loginId,name,email?,departmentId,role?,workplaceId?,approverUserId?} 1名追加 → 部署アプリへプロビジョニング実行 → 結果返却
// PUT    {id,name?,email?,departmentId?,workplaceId?,role?,approverUserId?} 更新 → 再プロビジョニング（権限・承認者の変更を各アプリへ反映）
    ...
    if (req.method === "GET") {
      const users = await sql`
        SELECT u.id, u.login_id, u.name, u.email, u.department_id, u.role,
               u.workplace_id, u.approver_user_id, u.created_at,
